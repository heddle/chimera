package cnuphys.mosaic.grid.mapping;
...
public class ArchimedesProjection implements IMapProjection {

	private static final double MAX_LAT = Math.toRadians(89); // Avoid poles (85° limit)

	private double centralLon = 0.0;

	@Override
	public void latLonToXY(Double latLon, Double xy) {
		...
		// Archimedes projection formula
		xy.x = lon - centralLon;
		xy.y = 2 * Math.sin(lat);
	}

	@Override
	public void latLonFromXY(Double latLon, Double xy) {
		double x = xy.x;
		double y = xy.y;

		// Inverse Archimedes projection formula
		latLon.x = x + centralLon; // Longitude
		latLon.y = Math.asin(y / 2); // Latitude
	}
	...
	@Override
	public boolean isPointOnMap(Double xy) {
		double lat = Math.asin(xy.y / 2); // Latitude // Check if the point's Archimedes latitude is within valid range
	...
	@Override
	public boolean isPointVisible(Double latLon) {
		double lat = latLon.y;
