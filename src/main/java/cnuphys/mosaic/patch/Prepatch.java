...
//		//they should form a valid loop
	...
	/**
	 * Get the theta curves that will be used to
	 * make theta patches
	 *
			...
			while (!tvals.isEmpty()) {
				...
				for (int i = 0; i < tvals.size(); i++) {
					...
					if (Math.abs(theta1 - theta0) < TOL) {
                        ...
                        ThetaCurve tc = ThetaCurve.createThetaCurve(this, theta0, phi0, phi1, R);
		...
		if (_thetaPatches == null) {
			...
			if (thetaCurves.isEmpty()) {
				...
				while (!allThetaCurves.isEmpty()) {
					System.out.println("\nTheta Curves: " + allThetaCurves.size());
					List<BaseCurve> patchCurves = new ArrayList<>();
					ThetaCurve thetaCurve = allThetaCurves.remove(0);
					System.out.println("\nNew Theta Curve: p0: " + thetaCurve.getP0() + " p1: " + thetaCurve.getP1()
							+ "   length: " + thetaCurve.pathLength());
					...
					while (!done) {
						...
						if (currentCurve instanceof GeneralCurve) {
							for (ThetaCurve tc : allThetaCurves) {
								if (curvesConnect(currentCurve, tc)) {
									...
									foundCurve = true;
									patchCurves.add(tc);
									System.out.println("Connected to theta curve: p0: " + tc.getP0() + " p1: "
											+ tc.getP1() + "   length: " + tc.pathLength());
						...
						if (!foundCurve) {
							for (BaseCurve splitCurve : allSplitCurves) {
								if (curvesConnect(currentCurve, splitCurve)) {
									...
									foundCurve = true;
									System.out.println("Connected to split curve: p0: " + splitCurve.getP0() + " p1: "
											+ splitCurve.getP1() + " theta index: "
											+ getSplitCurveThetaIndex(splitCurve) + "  length: " + splitCurve.pathLength());
						...
						if (!foundCurve) {
							...
							success = false;
							System.err.println("No curve found to connect to, bad patch");
						}

					}
					if (success) {
						System.out.println("Theta Patch curves: " + patchCurves.size());
				...
				System.out.println("Theta Patches Count: " + _thetaPatches.size());
			} //else 
		}
